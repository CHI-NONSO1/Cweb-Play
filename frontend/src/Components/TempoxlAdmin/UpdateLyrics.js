...
const history = useNavigate()
  ...
  useEffect(() => {
    ...
    const GetLyrics = async () => {
      ...
      const lyrics = await axios.get(`http://localhost:8000/lyrics/one/${lyricsid}`);
      ...
      setSongid(lyrics.data.songid)
     setSongtitle(lyrics.data.songtitle)
    }
  ...
  const [line70, setLine70] = useState('');
      ...
      const addLyrics = async (e) => {
        ...
        try{
          ...
          await axios.patch(`http://localhost:8000/lyrics/${lyricsid}`, 
